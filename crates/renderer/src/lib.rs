...
use renderer_schema::{Color, EffectV1, MAX_CANVAS_DIMENSION, NodeKindV1, SceneV1};
#[cfg(test)]
use std::path::PathBuf;
use std::{collections::HashMap, fs, fs::File, path::Path, sync::mpsc};
...
use analytic_geometry::*;
use animation::*;
use assets::*;
pub use error::{RenderError, RenderedImage};
use gif::*;
use limits::{MAX_COMPOSITION_TEXTURE_PIXELS, MAX_GPU_TEXTURE_DIMENSION};
// Only the `#[cfg(test)]` reference rasterizer below still uses these; they move with it.
#[cfg(test)]
use limits::{
    MAX_ASSET_BYTES, MAX_ASSET_PIXELS, MAX_GLYPH_SIZE, MAX_TEXT_BYTES, MAX_TEXT_GLYPHS,
